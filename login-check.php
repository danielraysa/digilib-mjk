<?php
    ...
    if(mysqli_num_rows($check) == 1 || mysqli_num_rows($check_kar) == 1){
        if(mysqli_num_rows($check) == 1){
            ...
            if($point != 0){
                $insert_point = mysqli_query($conn, "INSERT INTO point_pengguna (id_ppengguna, id_pengguna, id_point, tgl_perolehan) SELECT IFNULL(MAX(id_ppengguna)+1,1), '".$data['id_pengguna']."', ".$point.",'".date('Y-m-d H:i:s')."' FROM point_pengguna") or die(mysqli_error($conn));
            }
